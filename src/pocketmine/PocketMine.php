...
namespace pocketmine {
	...
	const BUILD = "";
	const VERSION = "10.3.34.5";
	const API_VERSION = "1.0.0";
	const CODENAME = "PINEAPPLE LAWYER";
	const MINECRAFT_VERSION = "iNET v10.3.34.5";
	...
	if(php_sapi_name() !== "cli"){
		$logger->critical("You must run iNET using the CLI.");
		++$errors;
	}
	...
	if(extension_loaded("pocketmine")){
		if(version_compare(phpversion("pocketmine"), "0.0.1") < 0){
			$logger->critical("You have the native iNET extension, but your version is lower than 0.0.1.");
			++$errors;
		}elseif(version_compare(phpversion("pocketmine"), "0.0.4") > 0){
			$logger->critical("You have the native iNET extension, but your version is higher than 0.0.4.");
			++$errors;
		}
	}
	...
	if(\Phar::running(true) === ""){
		$logger->warning("Non-packaged iNET installation detected, do not use on production.");
	}
	...
	echo("[iNET] Service shutdown detected. Please restart the console.");
